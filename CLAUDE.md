...
Every comment consumes tokens in future context windows. The next agent reading this code will not have your current context window to reference BUT WILL be able to see the code. Write comments only when the code is unclear, has a gotcha that might be overlooked, or has intent that needs explanation not obvious from the code itself. Avoid comments that merely restate the obvious about WHAT the code is or does.
