...
| `--seed INT` | No | Random seed for reproducible generation. Enables seed walking with `--max-chains` (see explore). Only with `--generated` |
...
| `--seed INT` | No | Random seed for reproducible test generation. In stateful mode with `--max-chains`, enables seed walking (see below) |
...
| `--max-chains INT` | No | Maximum chains to generate in stateful mode (default: 20). With `--seed`, enables seed walking |
...
**Seed walking (stateful mode):** When both `--seed` and `--max-chains` are specified, if the initial seed generates fewer unique chains than requested, api-parity automatically tries incrementing seeds (seed+1, seed+2, ...) until enough chains are accumulated. Chains are deduplicated by operation sequence. This enables more thorough exploration of APIs with complex link graphs. Up to 100 seeds are tried before stopping. The output reports which seeds were used.
