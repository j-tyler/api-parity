...
def _extract_operation_id(bundle_path: Path, bundle_type: BundleType) -> str:
    """Extract operation_id from case.json or chain.json without full Pydantic validation.

    For chain bundles, uses the first step's operation_id as the canonical ID.
    The mismatch_step is already captured in the dedup key via diff_data, so
    the operation_id here is just for grouping context.
...
def merge_bundles(input_dirs: list[Path], output_dir: Path) -> MergeSummary:
    ...
    for input_dir in input_dirs:
        ...
        for bundle_path in bundles:
            ...
            try:
                ...
                operation_id = _extract_operation_id(bundle_path, bundle_type)
    ...
    if dedup_groups:
        ...
        for bundle_info in dedup_groups.values():
            ...
            shutil.copytree(bundle_info.path, dest)
