...
Binary responses stored as base64 strings (already in Executor) and compared via CEL string operations. Reuses existing infrastructure. Predefineds: `binary_exact_match`, `binary_length_match`, `binary_nonempty`. If `binary_rule` not specified, binaries aren't compared (backward compatibility).
...
Synthetic response generation for chain discovery produces values satisfying OpenAPI schema constraints. Critical for enum-constrained parameters—UUID placeholders would be rejected. `SchemaValueGenerator` handles enum (first value), const, format (uuid/date-time/uri/email), and type constraints. Graceful fallback to UUID when no schema found.
